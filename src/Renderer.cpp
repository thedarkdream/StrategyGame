...
void Renderer::renderUnitPanel(Game& game) {
    ...
    bool isEnemyInspection = !player.hasSelection() && inspectedEnemy;
    sf::Color panelOutlineColor = isEnemyInspection ? sf::Color(180, 60, 60) : sf::Color(100, 100, 100);
    ...
    // Build name string - for enemy, show "Enemy" prefix; for own units, show count
    std::string nameStr;
    ...
    nameText.setFillColor(isEnemyInspection ? sf::Color(255, 150, 150) : sf::Color::White);
    ...
    // Show hotkeys (only for own units)
    if (!isEnemyInspection) {
